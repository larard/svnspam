...
def blurt(text)
  $stderr.puts("svnspam.rb: #{text}") if $debug
end
...
# Represents a top-level directory in the repository 
# this class is named incorrectly).  Collects a list of
...
class FileEntry
  ...
  # records whether 'svn diff' reported this as a binary file
...
# web SVN interfaces (e.g. SVNweb).  Subclasses overide these methods to
...
class NoFrontend
  ...
  # should turn this into a link to a webpage view of this SVN directory
  def path(path, tag)
    htmlEncode(path)
  end
...
# Superclass for objects that can link to SVN frontends on the web (ViewSVN,
# etc.).
class WebFrontend < NoFrontend
 ...
 protected
  def add_repo(url)
    if @repository_name
      if url =~ /\?/
        "#{url}&amp;svnroot=#{urlEncode(@repository_name)}"
      else
        "#{url}?svnroot=#{urlEncode(@repository_name)}"
      end
    else
...
# Link to ViewSVN
class ViewSVNFrontend < WebFrontend
  def initialize(base_url)
    super(base_url)
  end
  ...
  def version_url(path, version)
    add_repo("#{@base_url}#{urlEncode(path)}?rev=#{version}&amp;content-type=text/vnd.viewsvn-markup")
  end
...
# Link to SVNweb
class SVNwebFrontend < WebFrontend
  def path_url(path, tag)
    if tag == nil
      add_repo(@base_url + urlEncode(path))
    else
  ...
  def version_url(path, version)
    add_repo("#{@base_url}#{urlEncode(path)}?rev=#{version}&amp;content-type=text/x-svnweb-markup")
  end
...
# Handle lines from LogReader that are the output from 'svn diff -u' for the
...
if ARGV.length != 1
  ...
  puts "Usage: svnspam.rb [ --to <email> ] [ --config <file> ] <collect_diffs file>"
  exit(-1)
end
...
if FileTest.exists?($config)
  blurt("Using config '#{$config}'")
  load $config
else
  blurt("Config file '#{$config}' not found, ignoring")
end
...
if $viewsvnURL != nil
  $viewsvnURL << "/" unless $viewsvnURL =~ /\/$/
  $frontend = ViewSVNFrontend.new($viewsvnURL)
elsif $svnwebURL !=nil
  $svnwebURL << "/" unless $svnwebURL =~ /\/$/
  $frontend = SVNwebFrontend.new($svnwebURL)
else
  $frontend = NoFrontend.new
end

if $viewsvnURL != nil || $svnwebURL !=nil
...
HEAD
  ...
  if $debug
    blurt("leaving file #{$logfile}.emailtmp")
  else
    File.unlink("#{$logfile}.emailtmp")
  end

  mail.puts("<center><small><a href=\"http://www.bitpusher.com/projects/svnspam/\" title=\"commit -&gt; email\">SVNspam</a> #{$version}</small></center>")
...
class SendmailMailer
  def send(from, recipients)
    ...
    IO.popen(cmd, "w") do |mail|
      ...
      if from
        blurt("Mail From: <#{from}>")
      else
        blurt("Mail From not set")
      end
...
class SMTPMailer
  ...
  def send(from, recipients)
    if from == nil
      from = EmailAddress.new(ENV['USER'] || ENV['USERNAME'] || 'svnspam')
    end  
    unless from.address =~ /@/
      from.address = "#{from.address}@#{ENV['HOSTNAME']||'localhost'}"
    end
    smtp = Net::SMTP.new(@smtp_host)
    blurt("connecting to '#{@smtp_host}'")
    smtp.start()
...
mailer.send($from_address, $recipients) do |mail|
  ...
  mail.header("X-Mailer", "SVNspam #{$version} <http://www.bitpusher.com/projects/svnspam/>")
